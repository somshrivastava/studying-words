...
export class SomTableComponent implements OnInit {
    ...
    fileReader.onload = () => {
      ...
      for (var line = 0; line < lines.length; line++) {
        this.columns.forEach((column) => {
          this.newData[column.key] =
            lines[line].split(',')[this.columns.indexOf(column)];
        });
        this.dataService.createFirestoreDocument(
          `${this.database}`,
          this.newData
        );
      }
  ...
  getTableData() {
    this.dataService
      ...
      .subscribe((collection) => {
        this.data = collection.map((document) => {
          return {
  ...
  updateTableData(document) {
    ...
    this.dataService.updateFirestoreDocument(
      `${this.database}`,
      documentID,
      document
    );
  ...
  deleteTableData(document) {
    this.dataService.deleteFirestoreDocument(
      `${this.database}`,
      document.documentID
    );
  }
  ...
  navigateUser(column, row) {
    if (column['navigateOnClick']) {
      column['navigationPathKey'].forEach((key) => {
        column['navigationPath'].push(row[key]);
      });
      this.router.navigate(column['navigationPath']);
      column['navigationPathKey'].forEach((key) => {
        column['navigationPath'].splice(
          column['navigationPath'].indexOf(key),
          1
        );
      });
    }
